...
-J-Xms128M -J-Xmx512m -J-server})
    ...
    describe file('/etc/init.d/play-java-sample') do
      ...
      it { should be_owned_by 'root' }
      it { should be_grouped_into 'root' }
      its(:content) { should match(%r{PLAY_DIST_HOME="/opt/play/zip/play-java-sample"}) }
      its(:content) { should match(%r{PLAY="/opt/play/zip/play-java-sample/bin/play-java-sample"}) }
      its(:content) { should match(/USER="play-java-sample"/) }
      its(:content) { should match(%r{PID_PATH="/var/run/play-java-sample"}) }
      its(:content) { should match(%r{PID_FILE="\$\{PID_PATH\}/play.pid"}) }
      its(:content) { should match(%r{CONFIG_FILE="/opt/play/zip/play-java-sample/conf/application.conf"}) }
      its(:content) { should match(/APP_ARGS="-J-Xms128M -J-Xmx512m -J-server"/) }
